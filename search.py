...
def randomRead(file, n):
    ...
    while i < n:
        ...
        name = [str(df['first'][row]), str(df['middle'][row]), str(df['last'][row]), str(df['suffix'][row])]
        rep = [' '.join(part.title() for part in name if part != 'nan')]
        rep.append(df['sab'][row])
        candidate = [rep, name[0], name[1], name[2], ' '.join(part for part in name if part != 'nan'), str(df['min_year'][row]), str(df['candid'][row]), str(row)]
        for c in range(1, 5):
...
def orderRead(file, n):
    ...
    row = 38800 # set to desired starting row in source data
...
@retry(wait = wait_random_exponential(min=45, max=75), stop = stop_after_attempt(6), 
    ...
    for cand in urls:
        if len(cand) == 8:
            try:
                ...
                continue
    df = pd.DataFrame(rawData, columns = ['Sources', 'First', 'Middle', 'Last', 
